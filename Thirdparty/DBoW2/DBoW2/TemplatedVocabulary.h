...
public:
	 ...
	 * @param words ids of words
	 */
	void getWordsFromLevel(int level, std::vector<WordId> &words) const;
...
// --------------------------------------------------------------------------
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::getWordsFromLevel(int level, std::vector<WordId> &words) const
{
	words.clear();
	int depth = 0;
	words.push_back(0);
	while( true )
	{
		...
		std::vector<unsigned int> levelNodeIds;
		for( const unsigned int currentNodeId : words )
		{
			const vector<NodeId> &child_ids = m_nodes[currentNodeId].children;
			for(auto cit = child_ids.begin(); cit != child_ids.end(); ++cit)
			{
				const Node &child_node = m_nodes[*cit];
				levelNodeIds.push_back(child_node.id);
			}
		}
		depth++;
